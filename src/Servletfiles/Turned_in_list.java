...
@WebServlet("/turned_in_list")
public class Turned_in_list extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rs=st.executeQuery("select name,email_id from `"+table+"` where status='YES'");
			ResultSet rs1=st1.executeQuery("select count(name) from `"+table+"` where status='YES'");
			rs.next();
			rs1.next();
			n=rs1.getInt(1);
			name=new String[n];
		...
		RequestDispatcher rd=request.getRequestDispatcher("turned_in_list.jsp");
